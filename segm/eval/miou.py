...
def process_batch(
    model, batch, window_size, window_stride, window_batch_size, model_cfg
):
    ims = batch["im"]
    ...
    im = F.interpolate(ims[-1], ori_shape, mode="bilinear", align_corners=False)
...
def main(
    ...
    if window_stride is None:
        ...
        # to align the mIOU in training log
        if dataset_name == 'cityscapes':
            window_stride = 512
